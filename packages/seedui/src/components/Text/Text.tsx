...
const TextFactory = (variant: TextVariants) => {
  ...
  return styled[tag]<Required<TextProps>>((props) => ({
    ...
    margin: 0,
    marginBottom: props.makeSomeSpace ? '0.4em' : 0,
...
>(({ children, variant = 'p', makeSomeSpace = false, ...allProps }, forwardedRef: ForwardedRef<HTMLDivElement>) => {
  const TextElement = TextFactory(variant);

  return (
    <TextElement ref={forwardedRef} makeSomeSpace={makeSomeSpace} variant={variant} {...allProps}>
      {children}
    </TextElement>
  );
});
