import { ModeToggleDemo } from '../../../components/content/ModeToggleDemo';
...
`ThemeProvider` and every component, color token, and box shadow automatically adapts.
...
import { ThemeProvider, Mode } from '@juliensouki/seedui';
...
function App() {
  ...
  return (
    <ThemeProvider mode={mode}>
      <button onClick={() => setMode((m) => (m === 'light' ? 'dark' : 'light'))}>Toggle mode</button>
    </ThemeProvider>
  );
}
```
...
```tsx
import { styled } from '@juliensouki/seedui';
